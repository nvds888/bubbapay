...
<div className="card card-normal mb-4 group hover:shadow-sm transition-shadow">
  <div className="flex items-center justify-between mb-4">
    ...
    <div className="w-8 h-8 rounded-lg bg-gradient-to-r from-teal-400 to-teal-600 flex items-center justify-center">
...
{!recoveryMode && stage === 'initial' && (
  <>
    {/* Show refresh button if network error, otherwise show sign button */}
    {error && (error.includes('Network') || error.includes('network') || error.includes('timeout')) ? (
      <button
        type="button"
        onClick={() => window.location.reload()}
        className={`btn-secondary py-3 px-4 font-medium ${
      ...
      <button
        ...
        disabled={isLoading || (currentEscrow && !currentEscrow.funded)}
